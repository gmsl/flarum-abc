...
exports.getNextMigrationName = void 0;
const pad = (val, len) => String(val).padStart(len, '0');
function getNextMigrationName(currNames, name) {
    ...
    for (const migration of currNames.sort().reverse()) {
        ...
        if (!match)
            continue;
        const now = new Date();
        if (Number.parseInt(match[1], 10) === now.getFullYear() &&
...
exports.getNextMigrationName = getNextMigrationName;
